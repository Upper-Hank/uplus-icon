...
A handoff includes the 24×24 canvas, transparent background, fixed `black`/`none` design-source paint, part names, target-size previews, and owner approval. `currentColor` is generated by the repository's private adapter and is not required in the design-tool export.
